...
const createLoginDoc = asyncHandler(async (req, res, next) => {
    ...
    if (alreadyLoggedIn) {
        return next(new ApiError(409, "You have already logged in today"));
    }
...
const updateLogoutInfo = asyncHandler(async (req, res, next) => {
    ...
    if (!loginDoc) {
        return next(new ApiError(404, "No login doc found for logout"));
    }
    ...
    if (!updatedInfo) {
        return next(
            new ApiError(500, "Something went wrong while updating logout info")
        );
    }
...
const findUserAllLoginDocs = asyncHandler(async (req, res, next) => {
    ...
    if (!startDate || !endDate) {
        return next(
            new ApiError(400, "Please provide both start and end dates")
        );
    }
